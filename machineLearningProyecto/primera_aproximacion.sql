...
CREATE OR REPLACE FUNCTION create_view(ruta route, intervalo time_pair) 
    ...
    DECLARE
    BEGIN
     RAISE NOTICE '%    % ', ruta, intervalo.first_time;
        EXECUTE('CREATE OR REPLACE VIEW vista_' || ruta.intersection_id || '_' || ruta.tollgate_id || '_' ||TO_CHAR(intervalo.first_time, 'HH24_MI')||'_'|| TO_CHAR(intervalo.second_time, 'HH24_MI')|| ' AS SELECT *
